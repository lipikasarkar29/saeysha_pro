...
function setup(){
   ...
   dog = createSprite(windowWidth/2, windowHeight-120)
...
if(gameState === START){
    ...
    for(var i=0; i<foodGroup.length;i++){
        if(dog.isTouching(foodGroup.get(i))){
    ...
    for(var i=0; i<birdGroup.length;i++){
        if(dog.isTouching(birdGroup.get(i))){
            life = life - 1
      ...
      gameState = END
        ...
        if(life >= 1){
            ...
            textSize(40)
            fill("cyan")
            text ("Try again . . .", windowWidth/2 -100, windowHeight/2 + 150)
            ...
            stroke("red")
            strokeWeight(3)
            text("Sorry! You lose", windowWidth/2 -120, windowHeight/2)
        }
    }
